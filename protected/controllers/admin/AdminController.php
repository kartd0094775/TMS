...
class AdminController extends ControllerAdminCrud {
	...
	public function getItem() {
		...
		if ($item) {
			$item['permissionJson'] = jsonDecode($item['permissionJson']);
			$item['buildingIDs'] = @explode(',', $item['buildingIDs']);

		}

		$viewData['item'] = $item;

		$floors = Floor::model() -> findAll();
	...
	public function actionUpdateDo() {
		...
		if (isset($_POST['isChangeApiKey'])) {
			$item['apiKey'] = $this -> md5(uniqid());
		}
		$floorIDs = post('floorIDs');
		if (is_array($floorIDs)) {
			$item['floorIDs'] = implode(',', $floorIDs);
		}

		$buildingIDs = post('buildingIDs');
		if (is_array($buildingIDs)) {
			$item['buildingIDs'] = implode(',', $buildingIDs);
		}
