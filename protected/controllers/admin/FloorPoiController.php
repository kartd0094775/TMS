...
class FloorPoiController extends ControllerAdminCrud {

	public $uploadPath = 'floor';
	...
	public function actionList() {
		$this -> checkPermission('read', 'floor');

		$id = get('id');

		$floor = Floor::model() -> pk($id);
	...
	public function getItem() {
		...
		$floor = Floor::model() -> findByPk($item['floorID']);
		$viewData['floor'] = $floor;
	...
	public function actionUpdate() {
		$this -> checkPermission('update', 'floor');
	...
	public function actionGetList() {
		...
		if (!empty($search['nameEnglish'])) {
			$c -> addCondition('t.nameEnglish LIKE :nameEnglish');
			$params[':name'] = '%' . $search['nameEnglish'] . '%';
		}

		if (!empty($search['number'])) {
			$c -> addCondition('t.number LIKE :number');
			$params[':name'] = '%' . $search['number'] . '%';
		}
		...
		if (!empty($search['priorityFrom'])) {
			$c -> addCondition('t.priorityFrom = :priorityFrom');
			$params[':priorityFrom'] = $search['priorityFrom'];
		}

		if (!empty($search['priorityTo'])) {
			$c -> addCondition('t.priorityTo = :priorityTo');
			$params[':priorityTo'] = $search['priorityTo'];
		}
		...
		foreach ($items as $x) {
			...
			if (isset($icons[$x['iconID']])) {
				$a['iconCode'] = $icons[$x['iconID']]['code'];
			}

			$a['buildingName'] = '';
			$a['buildingID'] = 0;
			$building = Building::model() -> findByPk($x['buildingID']);
			if ($building) {
		...
		$itemCount = FloorPoi::model() -> count($c);
	...
	public function actionUpdateDo() {
		...
		$item['nameEnglish'] = post('nameEnglish');
		$item['number'] = post('number');
		$item['name'] = post('name');
		$item['x'] = post('x');
		$item['y'] = post('y');
		$item['iconID'] = post('iconID');
		$item['lat'] = post('lat');
		$item['lng'] = post('lng');
		// $item['priority'] = post('priority');

		$item['content'] = post('content');
		$item['contentEnglish'] = post('contentEnglish');
		$item['priorityFrom'] = post('priorityFrom');
		$item['priorityTo'] = post('priorityTo');

		$item['publicFacilityID'] = post('publicFacilityID');
		$item['vendorID'] = post('vendorID');

		$item['url'] = post('url');

		// $item['photo360'] = $this -> processPhoto($item['photo360'], 'photo360');
		$item['photo360'] = post('photo360');
		...
		if ($isSaveSuccess) {
			...
			$this -> showAlert(t('Save success', 'main'), 'list?id=' . $item['floorID']);
	...
	//delete single item
	public function actionDeleteDo() {
		$this -> checkPermission('delete', 'floor');
		$responseCode = 'false';
		$id = $this -> getPost('id');
		$item = FloorPoi::model() -> findByPk($id);
		if ($item) {
