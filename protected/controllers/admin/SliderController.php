...
class SliderController extends ControllerAdminCrud {
	...
	public function actionGetList() {
		...
		$itemCount = Slider::model() -> count($c);
	...
	public function actionUpdateDo() {
		...
		$item['youtubeUrl'] = post('youtubeUrl');
		...
		if ($isSaveSuccess) {
			...
			//process youtube photo-------------------------------------------------------------------------------------------
			$photoYoutube = post('photoYoutube');
			$sequence = post('youtubeSequence');
			$youtube = post('youtube');
			$nameYoutube = post('nameYoutube');

			//sort data first

			$youtubeJson = null;
			if (is_array($photoYoutube)) {
				foreach ($photoYoutube as $k => $v) {

					$temp = null;
					$temp['photo'] = $photoYoutube[$k];
					$temp['name'] = $nameYoutube[$k];
					$temp['youtube'] = $youtube[$k];

					while (isset($youtubeJson[$sequence[$k]])) {
						$sequence[$k] += 1;
					}

					$item['photoMobile'] = $photoYoutube[$k];
					
					$temp['sequence'] = $sequence[$k];

					$youtubeJson[$sequence[$k]] = $temp;
			...
			$item['youtubeJson'] = json_encode($youtubeJson);
			$item -> update();
