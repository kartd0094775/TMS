...
class PositionFloorController extends ControllerAdminCrud {
	...
	public function actionOutside() {
		...
		$c -> order = 'id desc';
		$c -> limit = 20000;
		$items = Position::model() -> findAll($c);
	...
	public function actionView() {
		...
		$buildingCode = get('building');
		$floor = get('floor');
		$originFloor = get('floor');
		...
		$c -> params = array(':code' => $buildingCode);
		// $c -> order = 'id DESC';
		...
		if ($building) {
			...
			if ($floor) {
				...
				// $file = $this -> basePath . '/../upload/floor/' . $floor['svg'];

				$file = $this -> basePath . '/../resource/' . $building['code'] . '/map/' . $floor['floor'] . '.svg';

				$svgPath = $this -> baseUrl . '/resource/' . $building['code'] . '/map/' . $floor['floor'] . '.svg';
				$viewData['svgPath'] = $svgPath;
				...
				//get all deveices
				$c = new Criteria;
				$c -> condition = 'locZ = :locZ AND buildingCode = :buildingCode';
				$c -> params = array(':locZ' => $originFloor, ':buildingCode' => $buildingCode);
				$c -> select = 'DISTINCT(deviceID)';

				$devices = Position::model() -> findAll($c);
	...
	public function actionGetList() {
		...
		$itemCount = Position::model() -> count($c);
