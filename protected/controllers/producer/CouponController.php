...
class CouponController extends ControllerProducerCrud {
		...
		if (count($_FILES) > 0) {
			...
			foreach ($_FILES as $file) {
				...
				if (move_uploaded_file($file['tmp_name'], $fileImg)) {
					...
					$fileArray['url'] = $this -> baseUrl . '/upload/coupon/' . $fileName;
	...
	public function actionGetList() {
		...
		$itemCount = Coupon::model() -> count($c);
	...
	public function actionUpdateDo() {
		...
		$item['originPrice'] = post('originPrice');
		$item['specialPrice'] = post('specialPrice');
