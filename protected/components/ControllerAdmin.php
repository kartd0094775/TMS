...
class ControllerAdmin extends Controller {
	...
	protected function beforeAction($action) {
		...
		$this -> _controllerName = str_replace('admin/', '', $this -> controllerName);
		...
		if ($this -> isAdminLogin()) {
			...
			$controllers = null;
			$controllers[] = array('modelName' => 'Monitor', 'controllerName' => 'monitor');
			$controllers[] = array('modelName' => 'Building', 'controllerName' => 'building');
			$controllers[] = array('modelName' => 'Floor', 'controllerName' => 'floor');
			$controllers[] = array('modelName' => 'City', 'controllerName' => 'city');
			$controllers[] = array('modelName' => 'Company', 'controllerName' => 'company');
			$controllers[] = array('modelName' => 'Outdoor', 'controllerName' => 'outdoor');

			// $controllers[] = array('modelName' => 'Public Facility', 'controllerName' => 'publicFacility');
			// $controllers[] = array('modelName' => 'Vendor', 'controllerName' => 'vendor');
			$controllers[] = array('modelName' => 'POI Type', 'controllerName' => 'poiType');

			$controllers[] = array('modelName' => 'Version', 'controllerName' => 'versionLog');

			$controllers[] = array('modelName' => 'Questionary', 'controllerName' => 'questionary');

			$controllers[] = array('modelName' => 'Producer', 'controllerName' => 'producer');
			$controllers[] = array('modelName' => 'Banner', 'controllerName' => 'banner');

			$controllers[] = array('modelName' => 'Position Log', 'controllerName' => 'position');
			$controllers[] = array('modelName' => 'Position Log 2', 'controllerName' => 'position2');

			$controllers[] = array('modelName' => '使用者數量/調用次數', 'controllerName' => 'statistics');
			$controllers[] = array('modelName' => 'Admin', 'controllerName' => 'admin');
			...
			$this -> admin = $this -> getSession('admin');
	...
	public function getAdminID() {
		return Yii::app() -> session['adminID'];
	}

	public function getUrl($actionName, $controllerName = null, $prefix = 'admin') {
	...
	public function isAdminRole() {
		...
		if ($this -> admin['roleID'] == '1' || $this -> admin['roleID'] == '2') {
			$result = true;
	...
	public function isAdminLogin() {
		...
		if (isset(Yii::app() -> session['isAdminLogin'])) {
			if (Yii::app() -> session['isAdminLogin'] == true) {
				return true;
			} else {
				return false;
			}
	...
	public function getPermissionCss() {
		...
		// $items = UserPermission::model() -> findAll($criteria);
		//
		// $adminPermissionArray = null;
		// foreach ($items as $x) {
		// if ($x['create'] == '1') {
		// $adminPermissionArray[$x['controller']]['create'] = 1;
		// }
		// if ($x['read'] == '1') {
		// $adminPermissionArray[$x['controller']]['read'] = 1;
		// }
		// if ($x['update'] == '1') {
		// $adminPermissionArray[$x['controller']]['update'] = 1;
		// }
		// if ($x['delete'] == '1') {
		// $adminPermissionArray[$x['controller']]['delete'] = 1;
		// }
		// }
		...
		/*
		 $cache = new CXCache();
		 $userID = $this -> userID;
		 if (!$cache -> get('userPermission' . $userID)) {
		 $this -> resetPermission($userID);
		 }
		 * */

		// ls('userPermission'.$userID);
		// $permission = $cache -> get('userPermission' . $userID);
		...
		if ($this -> admin['roleID'] != 1) {
			...
			//
			// if (!isset($adminPermissionArray[$controller][$permissionItem])) {
			//
			// if ($controller == $this -> controllerName) {
			// $permissionCss .= '.' . $permissionItem . ' { display:none !important; } ';
			// }
			//
			// // $permissionCss .= '.' . $moduleItem . '_' . $permissionItem . ' { display:none !important; } ';
			// $permissionCss .= '.' . $controller . ucfirst($permissionItem) . ' { display:none !important; } ';
			// }
			// }
			//
		}
