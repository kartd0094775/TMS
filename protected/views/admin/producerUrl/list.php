...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i><span t>店家網址 - <?php print $producer['name']; ?></span></h2>
					...
					<table class="table table-bordered table-sortable table-striped">
						<thead>
							<tr>
								...
								<!-- <th class="sortable sort-alpha"  style="width:60px"></th> -->
								<th class="sortable sort-alpha" orderField="name" t>Name</th>
								<th class="sortable sort-alpha" orderField="typeID" t>Source</th>
								<th class="sortable sort-alpha" orderField="url" t>Url</th>
							...
							<tr>
								...
								<th >
										<input type="hidden" class="form-control search" name="producerID" value="<?php print get('id'); ?>" />
...
print $this -> printJson('producerUrlType', $this -> getType('producer.urlType'));
?>
...
<script>
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			html += '<td>' + v(x['id']) + '</td>';
			html += '<td>' + v(x['name']) + '</td>';
			html += '<td>' + v(producerUrlType[x['typeID']]) + '</td>';
			// html += '<td><img src="' + baseUrl + '/upload/product/' + v(x['photo']) + '" class="listThumb" /></td>';
			html += '<td><a target="_blank" href="' + v(x['url']) + '">' + v(x['url']) + '</a></td>';
