...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i><span t>問卷管理</span></h2>
...
<script>
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			html += '<td>' + v(x['id']) + '</td>';
			// html += '<td><a href="' + baseUrl + '/questionary/item?id=' + x['id'] + '" target="_blank">' + v(x['name']) + '</a></td>';
			html += '<td>' + v(x['name']) + '</td>';
			...
			html += '<a type="button" class="btn btn-info btn-xs delete" href="' + baseUrl + '/' + 'admin' + '/questionaryList/list?id=' + x['id'] + '" t>填單清單</a> ';
			//html += '<button type="button" class="btn btn-danger btn-xs delete" onclick="sendPsuh(\'' + x['id'] + '\', this)" t>發送PUSH</button> ';
			//html += '<button type="button" class="btn btn-info btn-xs delete" onclick="sendPsuh(\'' + x['id'] + '\', this)" t>輸出EXCEL</button> ';
			html += '</td>';
	...
	function sendPsuh(id) {

		if (confirm('確認發送PUSH?')) {
			toPage('sendPushDo?id=' + id);
