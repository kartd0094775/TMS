...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i><span t>User list</span></h2>
					...
					<table class="table table-bordered table-sortable table-striped">
						<thead>
							<tr>
								...
								<th class="sortable sort-alpha" orderField="email" t>帳號(email)</th>

								<th class="sortable sort-alpha" orderField="phoneMobile" t>電話</th>
								<th class="sortable sort-alpha" orderField="isIn" t>是否進駐</th>
								<th class="sortable sort-alpha" orderField="addressBuilding" t>進駐地址</th>

								<th class="sortable sort-alpha" orderField="createTime" t>申請時間</th>
								...
								<th class="sortable sort-alpha" orderField="isEmailVerify" t>驗證</th>
								<th class="sortable sort-alpha" orderField="memo" t>管理者備註</th>
							...
							<tr>
								...
								<th >
								<select name="isIn" class="chosen-select form-control search" style="min-width:60px;">
									<option value="">----</option>
									<?php
								...
								<th >
								<select name="isEmailVerify" class="chosen-select form-control search" style="min-width:60px;">
									<option value="">----</option>
									<?php
									print $this -> printTypeOption('is');
									?>
...
<script>
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			if (x['isIn'] == '1') {
				html += '<td>新北巿汐止區新台五路一段' + v(x['addressBuilding']) + '棟 ' + v(x['addressNumber']) + '號 ' + v(x['addressFloor']) + '樓 </td>';
			} else {
				html += '<td></td>';
			}

			html += '<td>' + v(x['createTime']) + '</td>';
			html += '<td>' + v(is[x['isEmailVerify']]) + '</td>';
			html += '<td>' + v(x['memo']) + '</td>';
