...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i> Outdoor list</h2>
				...
				<div class="tile-widget bg-transparent-black-2">
					<div class="row">
						<div class="col-md-6">
							...
							<a class="btn btn-default btn-primary read " href="<?php print $this -> url('viewer'); ?>" t>Viewer</a>
						</div>
					...
					<table class="table table-bordered table-sortable table-striped">
						<thead>
							...
							<tr>
								...
								<th>
								<select name="buildingID" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
								...
								<select name="typeID" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
									print $this -> printTypeOption('outdoor.type');
									?>
								</select></th>
...
<script>
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			html += '<td>' + v(type[x['typeID']]) + '</td>';
			html += '<td>' + v(x['name']) + '</td>';

			html += '<td>';
			html += '<a class="btn btn-default btn-success btn-xs update" href="' + baseUrl + '/' + controllerName + '/update?id=' + x['id'] + '" t>Edit</a> ';
			// html += '<a class="btn btn-default btn-info btn-xs buildingRead" href="' + baseUrl + '/' + 'admin/building' + '/list?id=' + x['id'] + '" t>Building</a> ';
