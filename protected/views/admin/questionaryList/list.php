...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i><span t>問卷項目管理 - <?php print $questionary['name']; ?></span></h2>
	...
	<div class="breadcrumbs">
		<ol class="breadcrumb">
			...
			<li class="active" t>
				問卷項目管理 - <?php print $questionary['name']; ?>
			</li>
		</ol>
	</div>
			...
			<section class="tile color transparent-black" id="list">
				...
				<div class="tile-widget bg-transparent-black-2">
					<div class="row">
						<div class="col-md-6">
							...
							<a class="btn btn-default btn-success create " href="<?php print $this -> url('createPushListDo?id=' . get('id')); ?>" t>產生PUSH清單</a>
							<button id="buttonPush" class="btn btn-default btn-danger create " onclick="sendPushDo()" t>發送PUSH</button>
							
							<a class="btn btn-default btn-warning export " href="<?php print $this -> url('exportExcelDo?id=' . $questionary['id']); ?>" t>匯出 Excel</a>
				...
				<div class="tile-body nopadding">
					...
					<table class="table table-bordered table-sortable table-striped">
						<thead>
							<tr>
								<th class="sortable sort-alpha" orderField="id" style="width:">ID</th>
								<th class="sortable sort-alpha" orderField="userName" t>User</th>
								
														<th class="sortable sort-alpha" orderField="isSend" t>是否送出</th>
														
								<th class="sortable sort-alpha" orderField="isSubmit" t>是否填單</th>
								<th class="sortable sort-alpha" orderField="deviceTypeID" t>裝置</th>
								<th class="sortable sort-alpha" orderField="deviceToken"  t>Token</th>
							...
							<tr>
								...
								<th >
										<input type="hidden" class="form-control search" name="questionaryID" value="<?php print get('id'); ?>" />
								...
								<th >
								<select name="isSend" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
								...
								<th >
								<select name="isSubmit" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
									print $this -> printTypeOption('is');
									?>
...
print $this -> printJson('device', $this -> getType('device'));
print $this -> printJson('id', get('id'));
?>
...
<script>
	function sendPushDo() {
		...
		if (confirm('確認發送PUSH?')) {
			$('#buttonPush').attr('disabled', true);
			$('#buttonPush').text('發送中....');
			var url = getUrl('sendPushDo?id=' + id);
			$.ajax({
				url : url,
				type : 'post',
				// dataType : 'json',
				...
				success : function(r) {
					...
					$('#buttonPush').attr('disabled', false);
					$('#buttonPush').text('發送PUSH');
	...
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			html += '<td>' + v(x['id']) + '</td>';
			html += '<td>' + v(x['userName']) + '</td>';
			// html += '<td>' + v(x['name']) + '</td>';
			// html += '<td>' + v(x['email']) + '</td>';
			// html += '<td>' + v(x['phoneMobile']) + '</td>';
			html += '<td>' + v(is[x['isSend']]) + '</td>';
			html += '<td>' + v(is[x['isSubmit']]) + '</td>';

			html += '<td>' + v(device[x['deviceTypeID']]) + '</td>';
			html += '<td>' + v(x['deviceToken']) + '</td>';
			...
			html += '<td>';
			html += '<a class="btn btn-default btn-success btn-xs read" href="' + baseUrl + '/' + controllerName + '/update?id=' + x['id'] + '" t>檢視</a> ';
