...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i><span t>問卷項目資料</span></h2>
	...
	<div class="breadcrumbs">
		<ol class="breadcrumb">
			...
			<li class="" >
				<a href="<?php print $this -> url('list?id=' . $questionary['id']); ?>" t>活動項目管理 - <?php print $questionary['name']; ?></a>
			</li>
...
if (is_array($jsonData)) {
	foreach ($jsonData as $k => $x) {
		...
		if (count($answers) > 0) {
			print '
<div class="form-group">
					<label for="fullname" class="col-sm-2 control-label" ><span t>' . $x['name'] . '</span><span class="must"></span></label>
					<div class="col-sm-10">
					<ol>
';
...
<!--
				<div class="form-group">
					<label for="fullname" class="col-sm-2 control-label" ><span t>問題</span><span class="must"></span></label>
					<div class="col-sm-10">
...
<script>
	...
	$(document).ready(function() {
		...
		for (var i in data.jsonData) {
			...
			if ( typeof (x) == 'string') {
				$('*[name="' + i + '"][value="' + x + '"]').prop('checked', true);

			} else {
				for (var jj in x) {
					$('*[name="' + i + '[]"][value="' + jj + '"]').prop('checked', true);
				}
