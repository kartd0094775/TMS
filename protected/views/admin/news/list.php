					...
					<table class="table table-bordered table-sortable table-striped">
						<thead>
							<tr>
								...
								<!-- <th class="sortable sort-alpha" orderField="name">Password</th> -->
								<th class="sortable sort-alpha" orderField="name" t>Name</th>
								<th class="sortable sort-alpha" orderField="date" t>發布日期</th>
								<!-- <th class="sortable sort-alpha" orderField="typeID" t>訊息分類 </th> -->
								<th class="sortable sort-alpha" orderField="isReply" t>啟用回應 </th>
								<!-- <th class="sortable sort-alpha" orderField="isActive" t>是否上架</th> -->
								<!-- <th class="sortable sort-alpha" orderField="isShowOnIndex" t>是否出現在首頁</th> -->
							...
							<tr>
								...
								<th >
								<input type="text" class="form-control datepicker search" name="date" />
								</th>
								
								<!-- <th >
								<select name="typeID" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
									print $this -> printTypeOption('message.type');
									?>
								</select></th> -->
							 
								<th >
								<select name="isReply" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
...
print $this -> printJson('messageType', $this -> getType('message.type'));
?>
<script>
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			html += '<td>' + v(x['id']) + '</td>';
			// html += '<td>' + v(x['password']) + '</td>';
			html += '<td><a href="' + getUrl('item?id=' + x['id'], 'nessage') + '" target="_blank">' + v(x['name']) + '</a></td>';
			
			
			html += '<td>' + v(x['date']) + '</td>';
			// html += '<td>' + v(messageType[x['typeID']]) + '</td>';
			html += '<td>' + v(is[x['isReply']]) + '</td>';
