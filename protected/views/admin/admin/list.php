...
<div class="pageheader">
	...
	<h2><i class="fa fa-file-o" style="line-height: 48px;padding-left: 2px;"></i><span t>Admin list</span></h2>
					...
					<table class="table table-bordered table-sortable table-striped">
						<thead>
							<tr>
								...
								<th class="sortable sort-alpha" orderField="account" t>Username</th>
								<!-- <th class="sortable sort-alpha" orderField="name">Password</th> -->
								<th class="sortable sort-alpha" orderField="nickname" t>Name</th>
								<th class="sortable sort-alpha" orderField="roleID" t>角色</th>
							...
							<tr>
								...
								<th >
								<input type="text" class="form-control search" name="account" />
								</th>
								<!-- <th >
								<input type="text" class="form-control search" name="password" />
								</th> -->
								<th >
								<input type="text" class="form-control search" name="nickname" />
								</th>
								<th >
								<select name="roleID" class="chosen-select form-control search" >
									<option value="">----</option>
									<?php
...
<?php
print $this -> printJson('role', $this -> getType('admin.role'));
?>
<script>
	function getListHtml(data) {
		...
		for (var i in data) {
			...
			html += '<td>' + v(x['id']) + '</td>';
			html += '<td>' + v(x['account']) + '</td>';
			// html += '<td>' + v(x['password']) + '</td>';
			html += '<td>' + v(x['nickname']) + '</td>';
			html += '<td>' + v(role[x['roleID']]) + '</td>';
