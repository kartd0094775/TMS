...
class Coupon extends _Model
{
	...
	public function rules()
	{
		...
		return array(
			array('id, updatedat, createdat', 'required'),
			array('status, isSticky, sort, mainType, subType, total, countSold, countClick, countFavor, originPrice, specialPrice', 'numerical', 'integerOnly'=>true),
			array('id, producerId', 'length', 'max'=>36),
			array('area, name', 'length', 'max'=>255),
			array('photo', 'length', 'max'=>1024),
			array('start, end, expire, description, limitation, availableStore', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, status, isSticky, sort, producerId, mainType, subType, area, name, total, countSold, countClick, countFavor, originPrice, specialPrice, start, end, expire, photo, description, limitation, availableStore, updatedat, createdat', 'safe', 'on'=>'search'),
		);
	}
