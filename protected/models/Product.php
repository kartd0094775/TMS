...
class Product extends _Model
{
	...
	public function rules()
	{
		...
		return array(
			array('producerID, createTime', 'required'),
			array('producerID', 'length', 'max'=>36),
			array('name, photo', 'length', 'max'=>255),
			array('content, photoJson', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, producerID, name, createTime, content, photo, photoJson', 'safe', 'on'=>'search'),
		);
	}
