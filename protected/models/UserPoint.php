 ...
 */
class UserPoint extends _Model
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('point, reason, consumerId, createdat', 'required'),
			array('point', 'numerical', 'integerOnly'=>true),
			array('reason', 'length', 'max'=>255),
			array('consumerId', 'length', 'max'=>36),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, point, reason, consumerId, createdat', 'safe', 'on'=>'search'),
		);
	}
